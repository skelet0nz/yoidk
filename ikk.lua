...
local CharReal = game:GetService("Players").LocalPlayer.Character or game:GetService("Players").LocalPlayer.CharacterAdded:Wait()
local pose = "Standing"
...
local animNames = { 
	...
	walk = 	{ 	
		{ id = "http://www.roblox.com/asset/?id=12558243004", weight = 10 } 
	}, 
	run = 	{
		{ id = "http://www.roblox.com/asset/?id=12558243004", weight = 10 } 
	}, 
...
local animator = if Humanoid then Humanoid else nil
if animator then
...
function getHeightScale()
	if CharReal.Humanoid then
		if not CharReal.Humanoid.AutomaticScalingEnabled then
			return 1
		end
		...
		if AnimationSpeedDampeningObject ~= nil then
			scale = 1 + (CharReal.Humanoid.HipHeight - HumanoidHipHeight) * AnimationSpeedDampeningObject.Value / HumanoidHipHeight
		end
		return scale
	end	
	return 1
...
function onRunning(speed)
	...
	local speedThreshold = movedDuringEmote and CharReal.Humanoid.WalkSpeed or 0.75
...
-- connect events
CharReal.Humanoid.Died:connect(onDied)
CharReal.Humanoid.Running:connect(onRunning)
CharReal.Humanoid.Jumping:connect(onJumping)
CharReal.Humanoid.Climbing:connect(onClimbing)
CharReal.Humanoid.GettingUp:connect(onGettingUp)
CharReal.Humanoid.FreeFalling:connect(onFreeFall)
CharReal.Humanoid.FallingDown:connect(onFallingDown)
CharReal.Humanoid.Seated:connect(onSeated)
CharReal.Humanoid.PlatformStanding:connect(onPlatformStanding)
CharReal.Humanoid.Swimming:connect(onSwimming)
